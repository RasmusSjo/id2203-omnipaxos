    ...
    T: Entry,
{
    ...
    fn unsynlog_get(&self, idx: usize) -> StorageResult<Option<T>> {
        Ok(self.unsynlog.get(&idx).cloned())
    }

    fn unsynlog_remove(&mut self, idx: usize) -> StorageResult<()> {
        self.unsynlog.remove(&idx);
        Ok(())
    }
